...
apps/
  vivi2d-com/           # deferred: see Phase 4 preconditions
```
...
The future website shell should live under `apps/vivi2d-com/` and consume
`docs/user/` as content. The main domain, `vivi2d.com`, should be a product
portal; the long-form user documentation should live on `docs.vivi2d.com`.
That keeps localized user docs versioned in Git while allowing the portal,
documentation routing, search index, and deployment config to evolve separately.
...
- Scaffold `apps/vivi2d-com/` only when a dedicated web-infra owner is named,
  the deployment target for `vivi2d.com` is chosen, and the PR defines the
  build, preview, and release-surface scan commands.
...
- `npm run check:docs-public-surface` scans root public docs, developer docs,
  ...
  copy, and generated `apps/vivi2d-com` route metadata if the website shell is
  scaffolded.
- `check:docs-public-surface` is included in the standard release-quality gate
  before public user-doc release.
- If `apps/vivi2d-com/` is scaffolded, the PR names the web-infra owner,
...
- Website scaffolding, if added, consumes `docs/user/` without exposing
  `docs/backlog/`.
...
- user-doc media manifests, captions, alt text, and transcript files under
  `docs/user/assets/**`
- generated route metadata from `apps/vivi2d-com`, if scaffolded
...
- `apps/vivi2d-com/` must not be scaffolded until a web-infra owner is named,
  the deployment target is chosen, and build, preview, and release-surface scan
  commands are defined in the scaffolding PR.
