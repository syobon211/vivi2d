...
- Contributors should not hand-write source hashes. Use
  `npm run docs:user:rehash -- --slug=<route>` to preview an explicit route
  batch, then add `--write` to refresh stale localized pages as drafts. The
  helper shares the checker's parsing and hash normalization, never grants
  review approval, and refuses published-route downgrades. See the
  [media hash-refresh procedure](user-docs-media-production.md#translation-and-source-hash-behavior).
- A matching `sourceContentHash` binds metadata to the current English source
  and referenced media; it does not prove that a human reviewed the translation
  or replace authorized reviewer ownership.
...
4. Preview the explicit affected slugs with `npm run docs:user:rehash --
   --slug=<route>`, then add `--write`. Stale pages become drafts with cleared
   review approvals; the source hash is generated by the checker's shared code.
5. Require authorized locale review to restore the review flag, reviewer, and
   `reviewDate` before the page can return to `status: "reviewed"`.
