...
- npm dist-tag: `alpha`
- `latest` status: `<latest-status>`. During alpha, `latest` may mirror this
  version only as an explicit owner-reviewed convenience alias pointing to the
  current alpha; it is not a stable API commitment.
