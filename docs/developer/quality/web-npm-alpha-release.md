...
| CycloneDX SBOM | release workflow | Current: 14-day workflow artifact; durable GitHub release attachment remains open |
| `THIRD_PARTY_NOTICES` | repository | Tracked source and current 14-day workflow artifact; durable release attachment remains open |
...
- require `npm-alpha` approval for the validation/packaging job as well as the
  publish job, including hosted dry runs: packed artifacts are publicly hosted
  before npm publication. After tag verification and before build commands, run
  `node scripts/check-environment-protection.mjs --live --environment npm-alpha`
  with a step-local repository token. Unavailable/mismatched live policy or a
  failed generation/validation step must block uploads. Do not use
  `continue-on-error` or unconditional diagnostic artifact uploads.
- review public inputs/templates before authorizing generation and hosting,
  then emitted contents before later publication. Approval is not a content scan
  of not-yet-generated bytes; Actions artifacts are not private storage.
  ...
  validate-and-pack-web-alpha:
    ...
    steps:
      ...
      - name: Verify live release environment before build
        ...
        run: node scripts/check-environment-protection.mjs --live --environment npm-alpha
      ...
      - run: gitleaks detect --source . --no-git --redact=100
      - run: gitleaks git --log-opts="--all" . --redact=100
...
`fetch-depth: 0`; otherwise `check:history-secrets` and `gitleaks git --log-opts="--all" . --redact=100`
...
10. A manual `workflow_dispatch` dry-run path (`dryRun: true`) that runs
   validation and packaging without `npm publish`. It does not run on PR or push
   triggers; those use the separate quality workflows.
...
13. `check:environment-protection` validates the tracked policy offline. Its
    explicit `--live --environment npm-alpha` mode verifies the actual GitHub
    policy before hosted artifact generation: the designated owner reviewer,
    `web-v*` release tags only, admin bypass disabled, and owner self-approval
    permitted. One manual owner approval is required; neither default local
    quality success nor multiple registered reviewers proves multiple approvals.
