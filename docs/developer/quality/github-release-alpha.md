...
- The artifact-producing validation job requires `desktop-installer-alpha`
  approval before it starts. Source and Windows installer releases share this
  `v*-alpha.*` approval boundary; the name does not limit it to installer bytes.
  After tag verification and before build commands, it runs
  `node scripts/check-environment-protection.mjs --live --environment desktop-installer-alpha`
  with a step-local repository token. An unavailable or mismatched live policy
  blocks artifact uploads; ordinary local quality checks remain offline.
...
- `actions/upload-artifact` stores the generated asset set for review with
  retention of at most 14 days, only after successful generation and validation.
  Producer steps must not use `continue-on-error`, and upload steps must not use
  `always()`. Review public inputs/templates before authorizing generation and
  hosting; this approval is not review of the not-yet-generated bytes. Review
  emitted contents before later publication. Actions artifacts are not private
  even when the later Release is a draft.
