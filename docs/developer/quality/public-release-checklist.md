...
- Windows installer alpha release tooling is implemented for `v0.1.0-alpha.3`:
  ...
  `scripts/generate-windows-installer-alpha-review-packet.mjs`. The current installer
...
- Full git history and hosted surfaces still need a final release-machine scan
  ...
  `gitleaks git --log-opts="--all" . --redact=100` must pass before
...
- [ ] After final squash or history rewrite, `gitleaks git --log-opts="--all" . --redact=100` passes on the exact branch that will be pushed public.
...
- [ ] Workflow artifacts do not include private paths, source image bytes,
  provider payloads, or unreviewed generated media. Review public inputs and
  templates before authorizing artifact generation/hosting, and the emitted
  contents before later publication. An allowed path is not a content scan.
- [x] `npm run check:workflow-artifact-safety` forbids automatic coverage, perf,
  screenshot, trace, and workflow-recording uploads from ordinary CI. Tests still
  run; local media review must use synthetic public fixtures.
- [x] Remaining artifact uploads are restricted to exact release producers,
  protected Environment approval before job start, and a successful live
  Environment check before build commands. Failed generation/validation must not
  be ignored. These are public hosting controls, not private storage or proof
  that every generated byte is safe. Release draft status does not hide Actions
  artifacts; existing hosted logs/artifacts still require review.
- [ ] Cloudflare automatic production deployment remains disabled by default.
  Reconnecting Git builds or deploying manually requires separate owner approval.
  Repository checks and CI evidence do not verify the live Cloudflare settings.
...
- [ ] Attach the CycloneDX SBOM to the release artifact set when a future npm
  ...
  workflow path are implemented; keep this as a recurring attachment gate for
  future release sets.
...
- [ ] `npm run check:sdk-unlock:web` passes before future changes to
  ...
  promotion status. This is not a recurring republish gate when only docs,
  release notes, or registry-status records change.
