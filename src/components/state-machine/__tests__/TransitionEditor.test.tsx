...
describe("TransitionEditor", () => {
  ...
  it("crossfade (transitionDuration) 入力変更でonUpdateが呼ばれる", () => {
    const { container } = renderEditor(createTransition({ transitionDuration: 0.5 }));
    ...
    expect(onUpdate).toHaveBeenLastCalledWith({ transitionDuration: 1.2 });
    fireEvent.change(crossfadeInput, { target: { value: "-5" } });
    expect(onUpdate).toHaveBeenLastCalledWith({ transitionDuration: 0 });
  });
