...
import { bumpProjectStructureVersion } from "@/stores/projectMutator";
...
export function usePlayback() {
        ...
        if (clip.imageSequenceTracks && clip.imageSequenceTracks.length > 0) {
          ...
          for (const [targetMeshId, imageId] of Object.entries(imgSeqMap)) {
            if (imageId !== targetMeshId) {
              ...
              if (srcCanvas && srcCanvas !== getTexture(targetMeshId)) {
                setTexture(targetMeshId, srcCanvas);
                textureChanged = true;
              }
            }
          }
          if (textureChanged) bumpProjectStructureVersion();
        }
  ...
  useEffect(() => {
    const controller = new TimelineAudioPreviewController(undefined, (message) => {
    ...
    const unsubscribeTimeline = useTimelineStore.subscribe(
      (state) => [state.activeClipId, state.currentFrame, state.isPlaying] as const,
      () => {
        syncAudio();
      },
    );
...
function applyBakedLipSync(
  ...
  for (const [parameterId, normalized] of Object.entries(values.parameterValues)) {
    const parameter = project.parameters.find((item) => item.id === parameterId);
...
function applyLipSync(
  ...
  if (config.targetParameterId) {
    const param = project.parameters.find((p) => p.id === config.targetParameterId);
    if (param) {
