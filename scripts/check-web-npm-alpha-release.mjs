...
function checkPackageMetadata() {
  ...
  if (webPackage.repository?.url !== "git+https://github.com/syobon211/vivi2d.git") {
    failures.push(
...
function checkRequiredFiles() {
  for (const file of [
    ...
    "docs/developer/quality/web-npm-alpha-pack-allowlist.json",
  ]) {
...
function checkWorkflowShape() {
  ...
  requireText("npm ci");
  requireText(
    'node scripts/publish-web-npm-alpha.mjs --pack-result web-pack-result.json --tarball "$PACKED_TARBALL" --version "$VERSION"',
  );
  requireText(
...
function checkWorkflowRequirements() {
  ...
  assertOrder(
    "node scripts/publish-web-npm-alpha.mjs",
...
function checkVerifierContracts() {
  ...
  for (const phrase of [
    ...
    "rev-parse",
    "HEAD",
    "web-npm-alpha-pack-allowlist.json",
    "packFiles",
    "maxTarballBytes",
    "maxUnpackedBytes",
  ]) {
  ...
  const recordBuilder = readText("scripts/record-web-npm-alpha-artifacts.mjs");
  for (const phrase of [
    "web-npm-alpha-pack-allowlist.json",
    "packFiles",
    "maxTarballBytes",
  ...
  const publisher = readText("scripts/publish-web-npm-alpha.mjs");
  for (const phrase of [
    "verify-web-npm-alpha-release-record.mjs",
    "npm",
    "publish",
    "--access",
    "public",
  ...
  if (!guard.includes("VIVI2D_VERIFIED_WEB_NPM_ALPHA_PUBLISH")) {
    failures.push(
      "scripts/guard-web-npm-alpha-direct-publish.mjs must require the publish wrapper env guard.",
    );
  }
  if (
    webPackage.scripts?.prepublishOnly !==
