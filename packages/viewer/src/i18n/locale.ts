...
function canonicalizeLocaleTag(value: string): string | null {
  ...
  if (CONTROL_OR_SPACE_RE.test(normalized)) return null;
...
function familyParts(tag: string): string[] {
...
export function normalizeLocale(value: unknown): Locale | null {
  ...
  const parts = familyParts(canonical);
...
export function resolveLocaleFromSources(sources: {
  ...
  if (url) {
    return { locale: url, source: "urlOverride", mayPersist: false };
  }
